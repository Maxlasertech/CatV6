...
run(function()
    ...
    OreScanner = vape.Categories.Minigames:CreateModule({
        Name = 'Ore Scanner',
        Function = function(callback)
            if callback then
                repeat task.wait() until store.map or not OreScanner.Enabled
                if not OreScanner.Enabled then return end

                local map = store.map
                if not map then return end

                local blocks = map:FindFirstChild('Blocks')
                if not blocks then return end

                local min, max = map:GetBoundingBox()
                local size = map:GetExtentsSize()
                local origin = min.Position - size / 2
                local step = ScanRadius.Value

                local scanned = 0
                local total = math.ceil(size.X / step) * math.ceil(size.Z / step)

                for x = 0, size.X, step do
                    for z = 0, size.Z, step do
                        if not OreScanner.Enabled then return end
                        local pos = origin + Vector3.new(x, size.Y / 2, z)
                        pcall(function()
                            lplr:RequestStreamAroundAsync(pos, ScanRadius.Value)
                        end)
                        scanned += 1
                        if scanned % 5 == 0 then
                            notif('Ore Scanner', math.floor(scanned / total * 100) .. '% scanned', 2, 'info')
                        end
                        task.wait(ScanSpeed.Value / 1000)
                    end
                end

                notif('Ore Scanner', 'Scan complete', 3, 'info')
                OreScanner:Toggle()
            end
        end,
        Tooltip = 'Streams the entire map to reveal all ores — use with Kit ESP'
    })

    ScanRadius = OreScanner:CreateSlider({
        Name = 'Scan Radius',
        Min = 50,
        Max = 500,
        Default = 200,
    ...
    ScanSpeed = OreScanner:CreateSlider({
        Name = 'Scan Speed',
        Min = 0,
        Max = 500,
        Default = 50,
        Suffix = 'ms',
        Tooltip = 'Delay between each stream request; lower is faster but may lag'
    })
end)
