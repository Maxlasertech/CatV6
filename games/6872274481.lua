...
run(function()
	...
	if not canDebug then
		local cheatenginelib = loadstring(downloadFile('fart/libraries/cheatenginelib.lua'), 'cheatenginelib')(vape, vapeEvents, entitylib)
		require = function(v) 
...
run(function()
	...
	local packages = httpService:JSONDecode(downloadFile('fart/profiles/packages.json'))	
...
run(function()
    ...
    local function getShopId()
        if not entitylib.isAlive then warn('[ShopClicker] not alive') return nil end
        local localPosition = entitylib.character.RootPart.Position
        local id
        ...
        if not id then warn('[ShopClicker] no shop NPC within 20 studs') end
        return id
    end

    local function getHoveredItem(touchPos)
        local mousepos = touchPos or (inputService:GetMouseLocation() - guiService:GetGuiInset())
        warn('[ShopClicker] getHoveredItem pos:', mousepos.X, mousepos.Y)
        local guiObjects = lplr.PlayerGui:GetGuiObjectsAtPosition(mousepos.X, mousepos.Y)
        warn('[ShopClicker] GUI objects at position:', #guiObjects)
        for i, v in guiObjects do
            warn('[ShopClicker] obj['..i..']:', v.Name, v.ClassName)
            local obj = v
            while obj and obj ~= lplr.PlayerGui do
                local itemType = obj.Name:match('^(.+)_ShopItemCard$')
                if itemType then
                    warn('[ShopClicker] found item:', itemType)
                    return itemType
                end
                obj = obj.Parent
            end
        end
        warn('[ShopClicker] no ShopItemCard found')
    end

    local function canBuy(item)
        if item.ignoredByKit and table.find(item.ignoredByKit, store.equippedKit or '') then warn('[ShopClicker] canBuy false: ignoredByKit') return false end
        if item.lockedByForge or item.disabled then warn('[ShopClicker] canBuy false: locked/disabled') return false end
        if item.require and item.require.teamUpgrade then
            if (bedwars.Store:getState().Bedwars.teamUpgrades[item.require.teamUpgrade.upgradeId] or -1) < item.require.teamUpgrade.lowestTierIndex then
                warn('[ShopClicker] canBuy false: teamUpgrade requirement not met')
                return false
            end
        end
        local currency = getItem(item.currency)
        local hasEnough = (currency and currency.amount or 0) >= item.price
        warn('[ShopClicker] canBuy:', item.itemType, '| currency:', item.currency, '| have:', currency and currency.amount or 0, '| need:', item.price, '| result:', hasEnough)
        return hasEnough
    end

    local function purchase(itemType, shopId)
        warn('[ShopClicker] purchase called:', itemType, 'shopId:', shopId)
        if bedwars.BedwarsShopController.alreadyPurchasedMap[itemType] ~= nil then warn('[ShopClicker] already purchased (tiered):', itemType) return end
        if not bedwars.ItemMeta[itemType] then warn('[ShopClicker] no ItemMeta for:', itemType) return end
        if not bedwars.ItemMeta[itemType].block then warn('[ShopClicker] not a block item:', itemType) return end

        local item = bedwars.Shop.getShopItem(itemType, lplr, {shopId = shopId})
        if not item then warn('[ShopClicker] getShopItem returned nil for:', itemType) return end
        if not canBuy(item) then warn('[ShopClicker] canBuy failed for:', itemType) return end

        warn('[ShopClicker] sending purchase request for:', itemType)
        bedwars.Client:Get('BedwarsPurchaseItem'):CallServerAsync({
            shopItem = item,
            shopId = shopId
        }):andThen(function(suc)
            warn('[ShopClicker] purchase result:', itemType, '=', suc)
    ...
    local function startClicking(itemType)
        warn('[ShopClicker] startClicking:', itemType)
        if clickThread then
        ...
        clickThread = task.spawn(function()
            repeat
                ...
                warn('[ShopClicker] loop | shopOpen:', shopOpen, '| shopLoaded:', shopLoaded, '| shopId:', shopId)
                if shopId then
                    purchase(itemType, shopId)
                end
                task.wait(1 / CPS.Value)
            until not holding
            warn('[ShopClicker] stopped clicking (released)')
            clickThread = nil
        end)
    end
        ...
        Function = function(callback)
            if callback then
                ShopQuickBuy:Clean(inputService.InputBegan:Connect(function(input)
                    warn('[ShopClicker] InputBegan:', input.UserInputType.Name, '| TouchEnabled:', inputService.TouchEnabled)
                    if input.UserInputType ~= Enum.UserInputType.MouseButton1 and input.UserInputType ~= Enum.UserInputType.Touch then return end
                    local shopOpen = bedwars.AppController:isAppOpen('BedwarsItemShopApp')
                    warn('[ShopClicker] shop open:', shopOpen)
                    ...
                    warn('[ShopClicker] touchPos:', touchPos and (touchPos.X..','..touchPos.Y) or 'nil (using mouse)')
                    local itemType = getHoveredItem(touchPos)
                    warn('[ShopClicker] hovered item:', itemType or 'NONE')
                    if not itemType then return end

                    holding = true
                    warn('[ShopClicker] holding, waiting', HoldDelay.Value, 'seconds...')
                    task.delay(HoldDelay.Value, function()
                        local recheck = getHoveredItem(touchPos)
                        warn('[ShopClicker] after delay recheck:', recheck, '| still holding:', holding)
                ...
                ShopQuickBuy:Clean(inputService.InputEnded:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                        warn('[ShopClicker] InputEnded - released')
                        holding = false
                    end
                end))
                ...
                ShopQuickBuy:Clean(inputService.InputChanged:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.Touch and input.UserInputState == Enum.UserInputState.Cancel then
                        warn('[ShopClicker] Touch cancelled')
                        holding = false
                    end
                end))
            else
    	...
    	Function = function(callback)
    		if callback then
    			...
    			bedwars.KnockbackUtil.applyKnockback = function(root, mass, dir, knockback, ...)
    				...
    				if
    					TargetCheck.Enabled
    					and not entitylib.EntityPosition({
    						Range = 22,
    						Part = 'RootPart',
    						Players = true,
    					})
...
run(function()
    ...
    vape.Components.HotbarList = function(optionsettings, children, api)
        ...
        textbuttonicon.Image = getcustomasset('fart/assets/new/add.png')
            ...
            local mounted = bedwars.Roact.mount(create('BillboardGui', {
                ...
                create('Frame', {
                    ...
                    create('ImageLabel', {
                        ...
                        BackgroundTransparency = 1,
                        Image = getcustomasset('fart/assets/new/blur.png'),
...
run(function()
    ...
    BedBreaking = vape.Categories.Minigames:CreateModule({
        ...
        Function = function(callback)
            if callback then
                ...
                BedBreaking:Clean(task.spawn(function()
                    ...
                    while BedBreaking.Enabled do
                        ...
                        if BreakNearest and BreakNearest.Enabled and NearestMode.Value == 'Mouse' then
                            ...
                            _breakerRayParams.FilterDescendantsInstances = {char, gameCamera}
                ...
                local beds = collection('bed', BedBreaking)
                local luckyblock = collection('LuckyBlock', BedBreaking)
                local ironores = collection('iron_ore_mesh_block', BedBreaking)
                customlist = collection('block', BedBreaking, function(tab, obj)
                    if table.find(Custom.ListEnabled, obj.Name) then
                        table.insert(tab, obj)
                    end
                end)
                ...
                repeat
                    task.wait(1 / UpdateRate.Value)
                    if not BedBreaking.Enabled then break end
                    if entitylib.isAlive then
                        local localPosition = entitylib.character.RootPart.Position
    ...
    Range = BedBreaking:CreateSlider({
        Name = 'Break range',
        Min = 1,
        Max = 30,
        Default = 30,
    ...
    BreakSpeed = BedBreaking:CreateSlider({
        Name = 'Break speed',
        Min = 0,
        Max = 0.3,
        Default = 0.25,
    ...
    UpdateRate = BedBreaking:CreateSlider({
        Name = 'Update rate',
        Min = 1,
        Max = 120,
        Default = 60,
    ...
    Custom = BedBreaking:CreateTextList({
        Name = 'Custom',
        Function = function()
            if not customlist then return end
            table.clear(customlist)
    ...
    Bed = BedBreaking:CreateToggle({Name = 'Break Bed', Default = true})
    LuckyBlock = BedBreaking:CreateToggle({Name = 'Break Lucky Block', Default = true})
    IronOre = BedBreaking:CreateToggle({Name = 'Break Iron Ore', Default = true})
    Tesla = BedBreaking:CreateToggle({Name = 'Break Tesla', Default = true})
    Hive = BedBreaking:CreateToggle({Name = 'Break Hive', Default = true})
    Pinata = BedBreaking:CreateToggle({Name = 'Break Pinata', Default = false})
    Crops = BedBreaking:CreateToggle({
        Name = 'Break Crops',
        Default = false,
        Tooltip = 'Breaks pumpkin, carrot and watermelon crops'
    })
    Effect = BedBreaking:CreateToggle({
        Name = 'Show Healthbar & Effects',
        Function = function(callback)
            if CustomHealth.Object then
                CustomHealth.Object.Visible = callback
    ...
    CustomHealth = BedBreaking:CreateToggle({Name = 'Custom Healthbar', Default = true, Darker = true})
    Animation = BedBreaking:CreateToggle({Name = 'Animation'})
    SelfBreak = BedBreaking:CreateToggle({Name = 'Self Break'})
    InstantBreak = BedBreaking:CreateToggle({Name = 'Instant Break'})
    AutoTool = BedBreaking:CreateToggle({
        Name = 'Auto Tool',
        Tooltip = 'Automatically switches to the best tool for breaking blocks'
    })
    LimitItem = BedBreaking:CreateToggle({
        Name = 'Limit to items',
        Tooltip = 'Only breaks when tools are held'
    })
    MouseDown = BedBreaking:CreateToggle({
        Name = 'Require Mouse Down',
        Tooltip = 'Only breaks blocks when holding left click'
    })
    YetiBreaker = BedBreaking:CreateToggle({
        Name = 'Yeti Breaker',
        Tooltip = 'Focuses on frozen blocks placed by Yeti kit'
    })
    RagnarBreaker = BedBreaking:CreateToggle({
        Name = 'Ragnar',
        Tooltip = 'Pops the Ragnar ability whenever breaking'
    })
    LayerBreak = BedBreaking:CreateToggle({
        Name = 'Layer Break',
        Default = true,
        Tooltip = 'Breaks outer blocks first before inner ones'
    })
    BreakNearest = BedBreaking:CreateToggle({
        Name = 'Break Nearest',
        Default = false,
    ...
    NearestMode = BedBreaking:CreateDropdown({
        Name = 'Nearest Mode',
        List = {'Character', 'Mouse'},
        Default = 'Character',
        Visible = false,
        Tooltip = 'Character = closest to your avatar, Mouse = block under cursor'
    })
    ShowPath = BedBreaking:CreateToggle({
        Name = 'Show Path',
        Default = true,
        Tooltip = 'Show path boxes when breaking blocks'
    })
    BlockHighlight = BedBreaking:CreateToggle({
    ...
    BreakerHighlightColor = BedBreaking:CreateColorSlider({
    				...
    				if entitylib.isAlive then
    					pcall(function()
    						...
    						if Deposit.Enabled and getItem('bee') then
								...
								if v:GetAttribute('PlacedByUserId') == lplr.UserId and (localPosition - v.Position).Magnitude <= DepositRange.Value then
    									pcall(function()
        ...
        Function = function(call)
            ...
            task.spawn(function()
                while DangerDeposit.Enabled do
                    ...
                    local hive = findOwnedHive()
                    if hive then depositAll(hive) end
                end
            end)
        end
    })
...
run(function()
    ...
    -- Try to read the Roblox Friend List panel for game mode (works without privacy issues)
    ...
    local function getPresence(userId)
        -- If they're in the same server, we know exactly where they are
        if pl:GetPlayerByUserId(userId) then
            return game.PlaceId == BEDWARS_LOBBY_PLACE and 'BedWars Lobby' or 'BedWars (same server)', 'lobby', nil
        end
        ...
        if t == 2 then
            -- Use TeleportService for exact place ID (works if friends)
            ...
            if placeId == BEDWARS_GAME_PLACE or uni == BEDWARS_UNIVERSE then
                -- Determine game mode from lastLocation
                local mode = loc
                if mode == '' or mode:lower() == 'bedwars' then
                    mode = 'Game'
                end
                return 'BedWars · ' .. mode, 'game', nil
            end

            -- Not in BedWars — show what they're playing
            local gameName = loc ~= '' and loc or 'In Game'
            return gameName, 'other', nil
        end
    ...
    local function openGui()
        ...
        local acFix = Instance.new('Frame')   -- square off bottom corners of accent
        ...
        local function doSearch()
            ...
            task.spawn(function()
                ...
                -- Try friend list first (gives gamemode without privacy issues)
                ...
                -- Hint if gamemode unknown (not friends, privacy on)
...
run(function()
    ...
    ClosestSilentAim = vape.Categories.Combat:CreateModule({
                ...
                repeat
                    ...
                    if sword then
                        ...
                        if ent then
                            ...
                            local current, delay = tick(), 1 / math.max(Hitreg.Value, 1)
        ...
        Tooltip = 'Silent Aim with Hitreg and dual FastHits',
    })
...
run(function()
    ...
    DropFiltered = ForceDrop:CreateButton({
        Name    = 'Drop All (filtered)',
        Tooltip = 'Force-drops every item in your inventory that matches the filter list',
        Function = function()
            if not entitylib.isAlive then return end
            local count = 0
            for _, item in store.inventory.inventory.items do
    ...
    ItemFilter = ForceDrop:CreateTextList({
        Name    = 'Item Filter',
