...
run(function()
    ...
    local function getGroupName(assetName)
        local group = assetName
        for tier in BaseSwords do
            group = group:gsub('_?' .. tier .. '_?', '')
        end
        group = group:gsub('_?sword_?', ''):gsub('^_+', ''):gsub('_+$', '')
        if group ~= '' then
            return group
        end
        return nil
    ...
    local function collectSwordSkins()
        ...
        for _, child in items:GetChildren() do
            ...
            if not child:FindFirstChild('Handle') or not name:find('sword') then continue end
            if BaseSwords[name] then continue end
            if name:find('pickaxe') then continue end

            local group = getGroupName(name)
            if group then
                if not Names[group] then
                    Names[group] = {}
    ...
    local function Added(Item, Parent)
        ...
        local skins = Names[SkinDropdown.Value]
        if not skins then return end

        for _, skin in skins do
            if not skin:find(Item.Name) then continue end

            local ItemSkin = replicatedStorage.Items:FindFirstChild(skin)
            if not ItemSkin then continue end
        ...
        Function = function(callback)
            if callback then
                ...
                SwordSkinRender:Clean(vapeEvents.InventoryHeldChanged.Event:Connect(function(Tool)
                    ...
                    table.clear(Saved)
                    if Tool and BaseSwords[Tool.Name] then
